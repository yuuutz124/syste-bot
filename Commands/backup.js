...
module.exports = class backup {
    ...
    async run(client, message, args) {
        try{
            ...
            [Support](https://discord.gg/yF7dX9R)`)
            .setColor("#a11616")
             ...
             await   message.guild.roles.filter(r => r.name !== message.guild.member(client.user.id).highestRole.name).forEach(r => {
                    if (r.comparePositionTo(message.guild.member(client.user.id).highestRole) > 0){
                        let havnthighest = new RichEmbed()
                            ...
                            .setDescription(`The DriveGroup Role Is Not The Highest Role In The Server , This May Cause Some Errors When Loading The Backup. !
...
`)
                ...
                $backup delete        حذف واحدة من النسخ الاحتياطية الخاصة بك
                $backup info          الحصول على معلومات حول نسخة احتياطية
                $backup list          الحصول على قائمة النسخ الاحتياطية الخاصة بك
                $backup load          تحميل نسخة احتياطية
                $backup purge         حذف جميع النسخ الاحتياطية الخاصة بك`)
                .addBlankField()
